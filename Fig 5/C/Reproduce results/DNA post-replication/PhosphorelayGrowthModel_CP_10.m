function [Ap, ts,kgrowths1, tss,tsers,tdivs]=PhosphorelayGrowthModel_CP_10(gr1)
...
% DNA_replication_v=[RepDuration_ori:0.3:RepDuration_ori+3];
% Cell_Cyc=Non_rep_ori+DNA_replication_v;

DNA_replication_v=RepDuration_ori;
Non_rep_ori=[Non_rep_ori:0.1:Non_rep_ori+2];
Cell_Cyc=Non_rep_ori+RepDuration_ori;
...
% Set Growth History
%CellCycDuration=rates(1,2);
%gr=(log(2)/CellCycDuration);

%kgs=logspace(log10(gr),log10(gr),12);

kgs=(log(2)./Cell_Cyc)
%kgrowths=[kgs];nrep=numel(kgrowths);
kgrowths=[kgs(1,1)*ones(1,1),(kgs)];nrep=numel(kgrowths);
Non_rep_ori=[Non_rep_ori(1,1)*ones(1,1), Non_rep_ori];
Cell_Cyc=[Cell_Cyc(1,1)*ones(1,1), Cell_Cyc];

% kgs=logspace(log10(0.05),log10(0.5),12);
...
for i=1:nrep
    par1=pars;
    CellCycDuration=Cell_Cyc(1,i);
    RepDuration=DNA_replication_v;
    % CellCycDuration=(log(2)/kgrowths(i));
    %RepDuration=fRepDuration(kgrowths(i));

    gr=(log(2)/CellCycDuration);

    par1(20)=gr;  %update dilution rate constant %CP
...
map=2.5;
area(tss,map*tsers,'FaceColor',0.9*ones(1,3),'EdgeColor','none');hold on;
plot(ts,Ap,'r');xlim([0 max(ts)]);ylim([0 map]);hold on;
line([tdivs;tdivs],[zeros(1,numel(tdivs));map*ones(1,numel(tdivs))],'LineStyle',':','Color','k');
