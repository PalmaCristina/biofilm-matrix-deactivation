...
[results_duration,timef_clean,resultsf_clean] = N_Activation_deactivation_03(total_cells,gr,data_struct_A.(fields{i}).timef,data_struct_A.(fields{i}).resultsf,theresh);
...
%% 
 ...
 for i=1:1:size(fields,1)
      [Inspect_times_02,P_ON] = Probabilities_A_vs_cycles_02(total_cells,gr,data_struct_A.(fields{i}).timef_clean,data_struct_A.(fields{i}).resultsf_clean,theresh);
 
 % Save the variables to the file
    if i==1
        Inspect_times_02_O_A=Inspect_times_02;
        P_ON_O_A=P_ON;
        filename = sprintf('DATA_O_ACTIVATION_%d.mat', theresh);
        save(filename, 'Inspect_times_02_O_A', 'P_ON_O_A');
    end

    if i==2
        Inspect_times_02_C_A=Inspect_times_02;
        P_ON_C_A=P_ON;
        filename = sprintf('DATA_C_ACTIVATION_%d.mat', theresh);
        save(filename, 'Inspect_times_02_C_A', 'P_ON_C_A');
    end
