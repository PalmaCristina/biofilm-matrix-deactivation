...
%% Generate random OFF states:
for off_state=1:1:total_cells
    species=zeros(1,23); species(1,17)=200;
...
for y=1:1:size(gr_v,2)
    ...
    %Activation
    [timef,resultsf,y1_v] = Activation_Probability(total_cells,gr,factor,x_ss_2,total_time);
    filename = ['Activation_', num2str(strrep(num2str(gr), '.', '_')), '.mat'];
    save(filename);
    clearvars -except total_cells total_time min_val max_val ap_val gr gr_v x_ss_2
    state=2
    gr


    %Constant Activation
    [timef,resultsf] = Constant_Activation_probability(total_cells,ap_val,gr,x_ss_2,total_time);
    filename = ['Constant_Activation', num2str(strrep(num2str(gr), '.', '_')), '.mat'];
    save(filename);
    clearvars -except total_cells total_time min_val max_val ap_val gr gr_v x_ss_2
    state=4
    gr
