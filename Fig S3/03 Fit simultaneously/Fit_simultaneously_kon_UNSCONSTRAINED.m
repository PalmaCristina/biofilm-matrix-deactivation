%load('Data_C_corrected_clean_DIAGONAL.mat')
%load('Data_C_A.mat')
load('DATA_C_ACTIVATION.mat')
%load('Data_C_D')
load('DATA_C_DEACTIVATION')

%load('Data_O_corrected_clean_DIAGONAL.mat')
%load('Data_O_A.mat')
...
fun3 = @(params, x) (params(3) / (params(3) + params(4))) * (1 - exp(-x * (params(3) + params(4))));
fun4 = @(params, x) (params(4) / (params(3) + params(4))) * (1 - exp(-x * (params(3) + params(4))));
...
initial_guess = [0.0294,0.0294,0.0777,0.1102]; % Replace with your initial guess
initial_guess = [0.01,0.01,0.4,0.3];
% Perform the fitting using fminsearch
lb = [0, 0, 0,0]; % Replace with your bounds
ub = [0.5,0.5, 0.5,0.5]; % Replace with your bounds
...
kon_best_c = best_params(1)
kon_best_o = best_params(3)
koff_best_c = best_params(2)
koff_best_o = best_params(4)

% Optionally, plot the data and the fits
figure(2);
%subplot(1, 2, 1);
s1=scatter(x1, y1, 'o','MarkerEdgeColor',[0.85,0.33,0.10],'MarkerFaceColor',[0.85,0.33,0.10],'MarkerFaceAlpha',0.03,'MarkerEdgeAlpha',0.03); hold on;
s2=plot(x1, fun1(best_params, x1), '-','Color',[0.85,0.33,0.10],'LineWidth',4); hold on;
% Calculate MSE
...
s3=scatter(x3, y3, 'o','MarkerEdgeColor',[0.00,0.45,0.74],'MarkerFaceColor',[0.00,0.45,0.74],'MarkerFaceAlpha',0.03,'MarkerEdgeAlpha',0.03); hold on;
s4=plot(x3, fun3(best_params, x3), '-','Color',[0.00,0.45,0.74],'LineWidth',4); hold on;
% Calculate MSE
...
figure(2);
%subplot(1, 2, 2);
scatter(x2, 1-y2, 'o','MarkerEdgeColor',[0.85,0.33,0.10],'MarkerFaceColor',[0.85,0.33,0.10],'MarkerFaceAlpha',0.03,'MarkerEdgeAlpha',0.03); hold on;
plot(x2, 1-fun2(best_params, x2), '-','Color',[0.85,0.33,0.10],'LineWidth',4); hold on;
% Calculate MSE
residuals = y2 - fun2(best_params, x2);      % Differences
mse_x2 = mean(residuals.^2) % Mean of squared differences

scatter(x4, 1-y4, 'o','MarkerEdgeColor',[0.00,0.45,0.74],'MarkerFaceColor',[0.00,0.45,0.74],'MarkerFaceAlpha',0.03,'MarkerEdgeAlpha',0.03); hold on;
plot(x4, 1-fun4(best_params, x4), '-','Color',[0.00,0.45,0.74],'LineWidth',4); hold on;
% Calculate MSE
residuals = y4 - fun4(best_params, x4);      % Differences
...
[fitresultC, gofx3] = createFit(x3,y3,best_params(3),best_params(4),1); hold on;
...
[fitresultC, gofx4] = createFit(x4,y4,best_params(3),best_params(4),2);hold on;
...
figure(2)
text('Units', 'normalized', 'Position', [0.7, 0.9], 'String', sprintf('kon_C = %.4f\n kon_O = %.4f\n koff_C = %.4f\n koff_O = %.4f\n R^{2} = %.2f\n R^{2} = %.2f\n R^{2} = %.2f\n R^{2} = %.2f', kon_best_c,kon_best_o, koff_best_c, koff_best_o, gofx1.rsquare, gofx2.rsquare, gofx3.rsquare, gofx4.rsquare), 'FontSize', 12, 'BackgroundColor', 'white', 'EdgeColor', 'black');
set(gca,'FontSize',15)
