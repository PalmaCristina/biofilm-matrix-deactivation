...
for i=1:1:size(fields,1)
[results_duration,timef_clean,resultsf_clean] = N_Activation_deactivation_03(total_cells,gr,data_struct_D.(fields{i}).timef_corrected,data_struct_D.(fields{i}).resultsf_corrected);
...
%% 
 ...
 for i=1:1:size(fields,1)
      [Inspect_times_02,P_OFF] = Probabilities_D_vs_cycles_02(total_cells,gr,data_struct_D.(fields{i}).timef_corrected_clean,data_struct_D.(fields{i}).resultsf_corrected_clean)

  % Save the variables to the file
    if i==1
        Inspect_times_02_O_D=Inspect_times_02;
        P_OFF_O_D=P_OFF;
        save('DATA_O_DEACTIVATION.mat', 'Inspect_times_02_O_D', 'P_OFF_O_D');
    end

    if i==2
        Inspect_times_02_C_D=Inspect_times_02;
        P_OFF_C_D=P_OFF;
        save('DATA_C_DEACTIVATION.mat', 'Inspect_times_02_C_D', 'P_OFF_C_D');
    end
