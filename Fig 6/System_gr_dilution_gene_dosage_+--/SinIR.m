...
Rt=x(7);
T=x(8);
dx(1)=(vi0+vi*(apd)/(Ki+apd))*fglobal(0.2,0.8)*400/8.3*1-kdi*I*I*2+kudi*Id*2-kdeg*I*1;
dx(2)=kb1*Id*R*2-kdeg*IR*1;
dx(3)=-kb1*Id*R*1+kdi*I*I*1-kudi*Id*1-kdeg*Id*1;
dx(4)=(vl0+vl*Kl/(Rt+Kl))*fglobal(0.2,0.3)*200/8.3*1-kb2*L*R*1+kdlr*LR*1-kdegl*L*1;
dx(5)=kb2*L*R*1-kdlr*LR*1-kdeg*LR*1;
dx(6)=vr*fglobal(0.2,0.8)*200/8.3*1-kb1*Id*R*1-kdr*R*R*2+kudr*Rt*2-kb2*L*R*1+kdlr*LR*1-kdeg*R*1;
dx(7)=kdr*R*R*1-kudr*Rt*1-kdeg*Rt*1;
dx(8)=vt*Kt/(Rt+Kt)*fglobal(0.2,0.4)*200/8.3*1-1*T*1;
dx=dx';
end
