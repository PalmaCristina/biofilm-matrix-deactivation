...
for i = 1:length(row_numbers)
    ...
    Ap_means(start_idx:end_idx) = mean(Ap(start_idx:end_idx));
...
map=max(Ap_f)*5;
%area(tss,map*tsers,'FaceColor',0.9*ones(1,3),'EdgeColor','none');hold on;
plot(ts,Ap_f,'Color',[0.8510    0.3255    0.0980],'Linewidth',3);xlim([0 max(ts)]);ylim([0 map]);set(gca,'FontSize',15)
line([tdivs;tdivs],[zeros(1,numel(tdivs));map*ones(1,numel(tdivs))],'LineStyle',':','Color','k');
...
subplot(313)
plot(x, Tap_value, 'Color',[0.8510    0.3255    0.0980],'Linewidth',3);hold on;set(gca,'FontSize',15)
ylabel('[TapA] (\muM)');
xlabel('Time (h)');
xlim([0 70])
ylim([0 1.2])
