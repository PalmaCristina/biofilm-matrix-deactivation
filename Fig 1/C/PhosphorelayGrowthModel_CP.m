...
kgs=logspace(log10(gr),log10(gr),20);
...
% subplot(211)
% map=0.5;box on;
% area(tss,map*tsers*factor,'FaceColor',0.9*ones(1,3),'EdgeColor','none');hold on;
...
map=2.5;
area(tss,map*tsers*factor,'FaceColor',0.9*ones(1,3),'EdgeColor','none');hold on;
plot(ts,Ap*factor,'r');xlim([0 max(ts)]);ylim([0 map]);
line([tdivs;tdivs],[zeros(1,numel(tdivs));map*ones(1,numel(tdivs))*factor],'LineStyle',':','Color','k');
